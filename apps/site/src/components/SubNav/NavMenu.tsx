  ...
  return (
    <NavigationMenu.Root className="relative z-[1] flex w-full justify-center">
                    ...
                    {list.map((subItem, subIdx) => (
                      <li key={subIdx} className="row-span-3 grid">
                        <NavigationMenu.Link asChild>
                          <Link
                            href={generateUrl(subItem)}
                            className="flex h-full w-full select-none flex-col justify-end rounded-[6px] bg-gradient-to-b p-[25px] no-underline outline-none hover:bg-gray-100 dark:hover:bg-gray-900"
                          >
                            {subItem.label}
                          </Link>
                        </NavigationMenu.Link>
      ...
      <div className="perspective-[2000px] absolute top-full left-0 flex w-full justify-center">
        <NavigationMenu.Viewport className="relative shadow-md mt-[10px] h-[var(--radix-navigation-menu-viewport-height)] w-full origin-[top_center] overflow-hidden rounded-[6px] bg-white dark:bg-gray-800 transition-[width,_height] duration-300 sm:w-[var(--radix-navigation-menu-viewport-width)]" />
      </div>
    </NavigationMenu.Root>
  )
}
